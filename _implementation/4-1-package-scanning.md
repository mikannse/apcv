...
> **范围界定**: CAP-1「工具面发现」完整定义是**四层**(框架 / MCP / 运行时内建 / 子Agent 继承)。本故事**只覆盖第①层「框架层」的多文件扩展**。MCP manifest、运行时内建工具、子 Agent 继承三层是独立的后续故事(4.2/4.3/4.4,尚未拆解),**不在本故事范围内**——不要顺带实现它们,那会造成范围蔓延。
